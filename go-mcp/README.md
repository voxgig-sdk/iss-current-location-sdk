# iss-current-location-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IssCurrentLocation SDK as
two agent tools — `iss-current-location_list` and `iss-current-location_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iss-current-location-mcp)
make build

# 2. Provide credentials via the environment
export ISS_CURRENT_LOCATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user iss-current-location \
  -- /absolute/path/to/iss-current-location-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./iss-current-location-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// iss-current-location_list: first page of records
{ "entity": "iss_location" }
{ "entity": "iss_location", "query": { } }

// iss-current-location_load: one record by id
{ "entity": "iss_location", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iss-current-location-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user iss-current-location \
     -- "$PWD"/dist/*/iss-current-location-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `iss-current-location_list` and `iss-current-location_load` tools now appear
   in new sessions. Ask the agent to *"list iss_location using iss-current-location"*
   and it calls `iss-current-location_list` with `{"entity":"iss_location"}`.
...
```sh
export ISS_CURRENT_LOCATION_APIKEY=sk_live_xxx            # API key
export ISS_CURRENT_LOCATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `iss-current-location_list` tool
...
### Call the `iss-current-location_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `iss-current-location_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `iss-current-location_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ISS_CURRENT_LOCATION_APIKEY` | API key sent with every request. |
| `ISS_CURRENT_LOCATION_BASE` | Optional override of the API base URL. |
...
```sh
./iss-current-location-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"iss-current-location_load","arguments":{"entity":"iss_location","query":{"id":1}}}}'
```
