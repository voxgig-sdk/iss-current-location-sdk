...
// IssCurrentLocation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IssCurrentLocationOmni::makeStructRunner(
    ...
    {
        return IssCurrentLocationOmni::objmodel(IssCurrentLocationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return IssCurrentLocationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return IssCurrentLocationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IssCurrentLocationOmni::structfix($now, Struct::undef()))
                        !== json_encode(IssCurrentLocationOmni::structfix($val, Struct::undef()))) {
