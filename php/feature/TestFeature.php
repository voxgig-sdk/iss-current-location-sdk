...
// IssCurrentLocation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class IssCurrentLocationTestFeature extends IssCurrentLocationBaseFeature
{
    ...
    public function init(IssCurrentLocationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (IssCurrentLocationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = IssCurrentLocationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (IssCurrentLocationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(IssCurrentLocationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = IssCurrentLocationParam::call($ctx, $k);
