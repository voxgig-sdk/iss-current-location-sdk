...
// IssCurrentLocation SDK retry feature
...
class IssCurrentLocationRetryFeature extends IssCurrentLocationBaseFeature
{
    ...
    public function init(IssCurrentLocationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IssCurrentLocationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IssCurrentLocationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IssCurrentLocationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
