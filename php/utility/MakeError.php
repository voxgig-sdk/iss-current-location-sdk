...
class IssCurrentLocationMakeError
{
    public static function call(?IssCurrentLocationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IssCurrentLocationContext([], null);
        }
        $op = $ctx->op ?? new IssCurrentLocationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IssCurrentLocationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IssCurrentLocationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IssCurrentLocationError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IssCurrentLocationError) {
