# iss-current-location-cli

boru-driven command-line client **and** interactive REPL for the IssCurrentLocation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/iss-current-location-cli)
make build

# 2. See usage (words, entities, env vars)
./iss-current-location-cli --help

# 3. Provide credentials once, via the environment
export ISS_CURRENT_LOCATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./iss-current-location-cli load 1 iss_location            # {id:1} shorthand
./iss-current-location-cli load '{id:1}' iss_location       # explicit match map

# 5. Override the API base URL for a single call
ISS_CURRENT_LOCATION_BASE=https://api.example.com ./iss-current-location-cli load 1 iss_location

# 6. No arguments -> interactive REPL
./iss-current-location-cli
iss-current-location> load 1 iss_location
iss-current-location> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/iss-current-location-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/iss-current-location-cli load 1 iss_location
   ```
...
```sh
./iss-current-location-cli load 1 iss_location          # scalar shorthand for {id:1}
./iss-current-location-cli load '{id:1}' iss_location     # explicit match map
```
...
./iss-current-location-cli load 1 iss_location
```
...
Run with no arguments to open a REPL (prompt `iss-current-location>`). Each line is
evaluated as its own boru expression:

```text
$ ./iss-current-location-cli
iss-current-location> load 1 iss_location
iss-current-location> /help
iss-current-location> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `iss_location`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/iss-current-location-cli`. |
...
(`./iss-current-location-cli <expr>`) and interactively (the REPL), and expressions compose the
