# IssCurrentLocation TypeScript SDK



The TypeScript SDK for the IssCurrentLocation API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.IssLocation()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `IssCurrentLocationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `IssCurrentLocationSDK.test(testopts?, sdkopts?)` | `IssCurrentLocationSDK` | Create a test-mode client. |
...
| `client` | `client(): IssCurrentLocationSDK` | Return the parent SDK client. |
...
```ts
const iss_location = await client.IssLocation().load()
```
...
├── src/
│   ├── IssCurrentLocationSDK.ts        # Main SDK class
