...
//     (\`new IssCurrentLocationSDK(...)\` / \`IssCurrentLocationSDK.test(...)\` become
//     \`IssCurrentLocationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IssCurrentLocationSDK'
...
const TEST_SEED = {"entity":{"iss_location":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IssCurrentLocationSDK(...)\` and any
// \`IssCurrentLocationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IssCurrentLocationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IssCurrentLocationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
