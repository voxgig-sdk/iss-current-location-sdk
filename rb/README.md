...
The Ruby SDK for the IssCurrentLocation API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.IssLocation(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.IssCurrentLocation(nil).load(
  { "id" => "test01" }, nil
)
