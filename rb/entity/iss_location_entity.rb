...
class IssLocationEntity
  ...
  def make
    opts = @_entopts.dup
    IssLocationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IssCurrentLocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [IssLocation, Hash] the current IssLocation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IssCurrentLocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of IssLocation fields)
  def match_get
  ...
  #
  # @param reqmatch [IssLocationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.IssLocation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [IssLocation, Hash] the loaded IssLocation; raises IssCurrentLocationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IssCurrentLocationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
