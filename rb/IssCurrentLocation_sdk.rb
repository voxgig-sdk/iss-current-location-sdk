...
require_relative 'IssCurrentLocation_types'
...
class IssCurrentLocationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IssCurrentLocationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IssCurrentLocationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IssCurrentLocationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IssCurrentLocationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IssCurrentLocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IssCurrentLocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IssCurrentLocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => IssCurrentLocationError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue IssCurrentLocationError => err
    ...
    if fetched.is_a?(Hash)
      status = IssCurrentLocationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IssCurrentLocationError.new(
        "graphql_error", "IssCurrentLocationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IssCurrentLocationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
