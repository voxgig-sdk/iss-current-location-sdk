...
from isscurrentlocation_sdk.isscurrentlocation_types import (
    IssLocation,
...
class IssLocationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return IssLocationEntity(self._client, opts)
    ...
    def match_get(self) -> IssLocation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> IssLocation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.IssLocation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
