# IssCurrentLocation SDK test runner

from __future__ import annotations
import os
import json

from isscurrentlocation_sdk.utility.voxgig_struct import voxgig_struct as vs


class IssCurrentLocationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IssCurrentLocationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IssCurrentLocationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IssCurrentLocationTestRunner.getenv("ISS_CURRENT_LOCATION_TEST_LIVE")
        override = IssCurrentLocationTestRunner.getenv("ISS_CURRENT_LOCATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IssCurrentLocationTestRunner.getenv(key)
        ...
        explain = IssCurrentLocationTestRunner.getenv("ISS_CURRENT_LOCATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IssCurrentLocationTestRunner._test_control is not None:
            return IssCurrentLocationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IssCurrentLocationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IssCurrentLocationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IssCurrentLocationTestRunner.load_test_control()
...
def load_env_local():
    IssCurrentLocationTestRunner.load_env_local()


def env_override(m):
    return IssCurrentLocationTestRunner.env_override(m)


def entity_data(v):
    return IssCurrentLocationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IssCurrentLocationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IssCurrentLocationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IssCurrentLocationTestRunner.load_test_control()


def live_client_options():
    return IssCurrentLocationTestRunner.live_client_options()


def live_delay_ms():
    return IssCurrentLocationTestRunner.live_delay_ms()
