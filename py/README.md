...
The Python SDK for the IssCurrentLocation API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.IssLocation(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.IssCurrentLocation(None).load(
    {"id": "test01"}, None
)
