...
func TestIssLocationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		issLocationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.iss_location")))
...
func iss_locationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ISS_CURRENT_LOCATION_TEST_ISS_LOCATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ISS_CURRENT_LOCATION_TEST_ISS_LOCATION_ENTID"])
