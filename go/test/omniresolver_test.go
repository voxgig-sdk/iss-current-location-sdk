...
import (
	...
	omni "github.com/voxgig-sdk/iss-current-location-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IssCurrentLocationError); is {
				out := map[string]any{
					"name":    "IssCurrentLocationError",
...
func omniCtx(arg any, client *sdk.IssCurrentLocationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IssCurrentLocationSDK); is {
			client = live
