...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ISSCURRENTLOCATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("ISSCURRENTLOCATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ISSCURRENTLOCATION_TEST_EXPLAIN"); explain != "" {
