...
module IssCurrentLocationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IssCurrentLocationResponse.new({})
    ...
    if fetch_err
      ...
      response = IssCurrentLocationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IssCurrentLocationResponse.new(fetched)
    else
