...
// IssCurrentLocation SDK pipeline test
//
...
{
    private static function utility(): IssCurrentLocationUtility
    {
        return new IssCurrentLocationUtility();
    }
    ...
    private static function ctx(array $over = []): IssCurrentLocationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IssCurrentLocationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IssCurrentLocationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IssCurrentLocationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IssCurrentLocationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IssCurrentLocationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IssCurrentLocationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IssCurrentLocationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IssCurrentLocationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IssCurrentLocationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IssCurrentLocationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IssCurrentLocationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IssCurrentLocationResponse([]),
            'result' => new IssCurrentLocationResult([]),
        ]);
        [, $err] = IssCurrentLocationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec([]),
            'response' => null,
            'result' => new IssCurrentLocationResult([]),
        ]);
        [, $err] = IssCurrentLocationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec(['step' => 's']),
            'response' => new IssCurrentLocationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IssCurrentLocationResult([]),
        ]);
        [, $err] = IssCurrentLocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec(['step' => 's']),
            'response' => new IssCurrentLocationResponse(self::resp(200, ['v' => 1])),
            'result' => new IssCurrentLocationResult([]),
        ]);
        [, $err] = IssCurrentLocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec(['step' => 's']),
            'response' => new IssCurrentLocationResponse(self::resp(200, ['v' => 2])),
            'result' => new IssCurrentLocationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IssCurrentLocationMakeResponse::call($ctx);
    ...
    {
        $preset = new IssCurrentLocationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec([]),
            'response' => new IssCurrentLocationResponse([]),
            'result' => new IssCurrentLocationResult([]),
        ...
        [$got, $err] = IssCurrentLocationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IssCurrentLocationResult([])]);
        [, $err] = IssCurrentLocationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IssCurrentLocationSpec([]), 'result' => null]);
        [, $err] = IssCurrentLocationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IssCurrentLocationSpec(['step' => 's']),
            'result' => new IssCurrentLocationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IssCurrentLocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IssCurrentLocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IssCurrentLocationSpec(['step' => 's']),
            'result' => new IssCurrentLocationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IssCurrentLocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IssCurrentLocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IssCurrentLocationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IssCurrentLocationSpec([]),
            'result' => new IssCurrentLocationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IssCurrentLocationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IssCurrentLocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IssCurrentLocationError('boom', 'boom');
        $utility->fetcher = function (IssCurrentLocationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IssCurrentLocationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IssCurrentLocationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IssCurrentLocationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IssCurrentLocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IssCurrentLocationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        IssCurrentLocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IssCurrentLocationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IssCurrentLocationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IssCurrentLocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IssCurrentLocationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IssCurrentLocationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IssCurrentLocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IssCurrentLocationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IssCurrentLocationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IssCurrentLocationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IssCurrentLocationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IssCurrentLocationResult(['ok' => false])]);
        $this->expectException(IssCurrentLocationError::class);
        IssCurrentLocationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IssCurrentLocationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IssCurrentLocationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IssCurrentLocationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IssCurrentLocationBaseFeature();
        $b = new IssCurrentLocationBaseFeature();
        IssCurrentLocationFeatureAdd::call($ctx, $a);
        IssCurrentLocationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IssCurrentLocationBaseFeature
    {
        $f = new IssCurrentLocationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IssCurrentLocationFeatureAdd::call($ctx, self::named_feature('a'));
        IssCurrentLocationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        IssCurrentLocationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        IssCurrentLocationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        IssCurrentLocationFeatureAdd::call($ctx, $replace);
        ...
        IssCurrentLocationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IssCurrentLocationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IssCurrentLocationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IssCurrentLocationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        IssCurrentLocationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IssCurrentLocationResponse(['headers' => null]),
            'result' => new IssCurrentLocationResult([]),
        ]);
        IssCurrentLocationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        IssCurrentLocationResultBody::call($ctx);
