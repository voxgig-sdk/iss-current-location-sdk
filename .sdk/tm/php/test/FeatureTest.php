...
// IssCurrentLocation SDK feature test
//
...
class FtCtrl extends IssCurrentLocationControl
{
...
class FtClient
{
    ...
    private IssCurrentLocationUtility $utility;

    public function __construct(IssCurrentLocationUtility $utility, array $options)
    {
    ...
    public function get_utility(): IssCurrentLocationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IssCurrentLocationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IssCurrentLocationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IssCurrentLocationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IssCurrentLocationConfig::make_feature($name);
    ...
    private static function buildUrl(IssCurrentLocationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IssCurrentLocationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IssCurrentLocationResponse($fetched);
        }
    ...
    private function fail(IssCurrentLocationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IssCurrentLocationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
